...
class Report {
  ...
  public:
    ...
    virtual void endPortfolio( Portfolio* anAccountable ) {};
    virtual void addAccount( Account* anAccountable ) {};
    virtual void endAccount( Account* anAccountable ) {};
...
class NiceReport : public Report {
  private:
    int prefix = 0;
  public:
    void addDeposit( Deposit* aDeposit );
    void addWithdraw( Withdraw* aWithdraw );
    void addPortfolio( Portfolio* anAccountable );
    void endPortfolio( Portfolio* anAccountable );
    void addAccount( Account* anAccountable );
    void endAccount( Account* anAccountable );
};
...
void NiceReport::addAccount( Account* anAccountable ) {
    report.emplace_back( string( prefix, ' ' ) + "Cuenta:" );
    prefix++;
}
void NiceReport::endAccount( Account* anAccountable ) {
    report.emplace_back( string( prefix, ' ' ) + "Total: (" + to_string( anAccountable->balance() ) + ")" );
    prefix--;
}
void NiceReport::addPortfolio( Portfolio* anAccountable ) {
    report.emplace_back( string( prefix, ' ' ) + "Portfolio:" );
    prefix++;
}
void NiceReport::endPortfolio( Portfolio* anAccountable ) {
    report.emplace_back( string( prefix, ' ' ) + "Total: (" + to_string( anAccountable->balance() ) + ")" );
    prefix--;
}
void NiceReport::addDeposit( Deposit* aDeposit ) {
    report.emplace_back( string( prefix, ' ' ) + "deposito de " + to_string( aDeposit->value() ) );
}
void NiceReport::addWithdraw( Withdraw* aWithdraw ){
    report.emplace_back( string( prefix, ' ' ) + "retiro de " + to_string( aWithdraw->value() ) );
}
